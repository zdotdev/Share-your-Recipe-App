...
async function editDish (body, id) {
  try {
    const data = await fetch(`http://localhost:3000/dish/editRecipe/${id}`, {
      method: 'PUT',
...
async function deleteDish (id) {
  try {
    const data = await fetch(`http://localhost:3000/dish/deleteRecipe/${id}`, {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
      body: ''
    })
...
async function addDish (newDish) {
  try {
    const data = await fetch('http://localhost:3000/dish/addRecipe', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
document
  ...
  .addEventListener('click', async () => {
    ...
    try {
      ...
      document.getElementById('edit-dish-ingredients').value =
        dish.dishes.dishIngredients
      document.getElementById('edit-dish-procedure').value =
        dish.dishes.dishProcedure
      console.log(data)
    } catch (err) {
      console.log(err)
    }
